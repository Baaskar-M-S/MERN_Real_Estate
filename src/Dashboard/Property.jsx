...
const Property = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        ...
        setData(data.filter(property => property._id !== id)); // Update local state to remove the deleted property
  ...
  return (
    <div className=" w-full my-12">
      ...
      <Link to='/propertycreate' className="text-blue-600 hover:underline mb-4 block">Create New Property +</Link>
              ...
              currentItems.map((property, index) => (
                <tr key={property._id} className="hover:bg-gray-50">
                  ...
                  <td className="py-2 px-4 flex flex-col gap-5">
                    <Link to={`/propertyedit/${property._id}`} className="text-blue-600 hover:underline">
                      Edit
                    </Link>
                    <button 
